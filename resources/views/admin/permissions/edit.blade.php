<x-admin-layout>
    ...
    <div class="container bg-white p-4 rounded-md">
        ...
        <div class="flex justify-end">
            <a href="{{ route('admin.permissions.index') }}" class="btn btn-dark btn-small">Permission Index</a>
            ...
            <div class="container">
                <div class="row justify-content-center">
                  <div class="col-sm-8">
                    ...
                    <div class="m-6 p-2">
                      <h3 class="font-semibold ">Permission Roles</h3>
                      <div class="flex space-x-2 inline mt-4 p2">
                        @if($permission->roles)
                        @foreach ($permission->roles as $permission_role)
                        
                        <form method="POST" action="{{ route('admin.permissions.roles.revoke', [$permission->id,$permission_role->id]) }}" onsubmit="return confirm('Are You Sure');" class="inline">
                          @csrf
                          @method('DELETE')
                          <button class="btn btn-danger btn-small mt-1" type="submit">{{ $permission_role->name }}</button>
                      </form>
                        @endforeach
                        @endif
                      </div>
